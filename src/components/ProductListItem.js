...
const ProductListItem = ({ product, addToCart, themeType, roundingMultiple, allowDecimals }) => {
  ...
  return (
    <div 
      className={`bg-[var(--color-bg-secondary)] rounded-lg shadow-md p-2 flex items-center gap-2 hover:shadow-xl transition-shadow duration-300 border border-[var(--color-border)] ${isGlowActive ? 'dark-glow' : ''} ${themeType === 'light' ? 'light-shadow' : ''}`}
    ...
    >
      ...
      <div className="flex-grow min-w-0">
        ...
        <div className="flex items-center justify-between mt-1">
          ...
          <p className="text-lg font-bold text-transparent bg-clip-text bg-gradient-to-r from-[var(--color-gradient-start)] to-[var(--color-gradient-end)] animate-gradient-x">
      ...
      <button 
        ...
        className="flex-shrink-0 bg-[var(--color-primary)] text-[var(--color-primary-text)] p-2 rounded-lg hover:bg-[var(--color-primary-hover)] transition-colors duration-200 disabled:bg-gray-400 disabled:cursor-not-allowed"
      >
